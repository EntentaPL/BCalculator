...
def inicjalizacjaEkranu(root):
    ...
    ekran = [
        tk.Label(root, bg="#000000", width=55, anchor="w", borderwidth=2)
        for i in range(3)
    ]
...
def inicjalizacjaPolaNaDane(root, ekran):
    ...
    pole_na_dane = tk.Entry(
        root, borderwidth=0, highlightcolor="black", highlightbackground="black"
    )
    pole_na_dane.grid(row=len(ekran), columnspan=6, ipadx=142, ipady=10)

    info = tk.Label(root, bg="black", width=55, anchor="w", borderwidth=2)
